...
<script  setup>
  const slotText = "{ <br/>default?: (args: FormPluginsProps<T>) => Component / null;<br/>row?: (args: FormPluginsProps<T>) => Component / null;<br/>label?: ({ label }: { label: string }) => Component / null;<br/>error?: ({ error }: { error: string }) => Component / null<br/>}"

  const TypeEnumText = "input / autocomplete / cascader / checkbox / color-picker / date-picker / mention / radio / rate / select / select-v2 / slider / switch / time-picker / time-select / transfer / tree  upload"
</script>
...
| slots                 | `Record<string, Fn>` <el-tooltip raw-content :content="slotText"> <itip /> </el-tooltip>                                       | `插槽`  | 可自定义渲染表单项或整行                              |
| type                  | `string` <el-tooltip :content="TypeEnumText"> <itip /> </el-tooltip>                                                           | -       | 渲染的组件类型                                        |
